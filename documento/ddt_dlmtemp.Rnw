...
plot1 <- d2 %>% mutate(dia2 = yday(fecha)) %>% 
  ggplot() + 
  geom_hex(aes(x=dia2, y=temp) ) +  
  geom_line(data = mutate(d1, dia2 = yday(fecha)),  aes(x=dia2,y=temp,group=serie)) +
  annotate("rect", xmin=156, xmax=160, ymin=-4 , ymax=30, alpha=0.2, color=NA, fill="red") +
...
plot1
@
\textcolor{en la figura: caption con descripcion, nombres de los ejes y la etiqueta de color, nombres de los paneles}
...
La ecuación de observación, explicita cómo es observado el proceso en tiempo $t$ dado el estado en el que se encuentra a tiempo $t$. En ambas ecuaciones, se puede introducir una componente aleatorio. Un diagrama de esto se puede ver en la figura \ref{diagrama}, un aspecto clave de estos modelos es que los datos observados, $Y_t$, son condicionalmente independientes dados los estados inobservables $\theta_t$, toda la dependencia pasa a través del proceso latente. \textcolor{blue}{relevancia de esto}
...
\textcolor{blue}{ejemplo de caso particular}
Caso particular: AR(2) $y_t = \phi_1 y_{t-1} + \phi_2 y_{t-2} + e_t$, definimos $\theta_t = 
\begin{pmatrix}
y_t \\
y_{t-1}
\end{pmatrix}
