...
dd <- tibble(t = seq(0,1, length.out = N), w = rnorm(N,0,.1), v = rnorm(N,0,.3) ) %>%
...
<<graf7c,fig.cap="Estimación de proceso de estados latentes en precencia de valores faltantes.", fig.height=5>>=
dd %>%
  select(t, yna, theta) %>%
  mutate(estimacion = smooth.na$s[-1] )  %>%
  gather( serie, valor, -t ) %>% 
  ggplot() + geom_line(aes(x=t,y=valor,col=serie)) +
  scale_color_manual( values=c("theta"='black',"yna"='grey','estimacion'='red')) +
  theme_bw()
@
