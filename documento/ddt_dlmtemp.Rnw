...
mod.tn <- filter(d2, serie == 'Tn') %>% mutate(temp = temp-mean(temp, na.rm = T)) %>% pull(temp) %>% 
  ts(start = c(1950, 1), frequency = 365) %>% 
  dlmMLE( parm = 0, build= parMLE)

# Estima el modelo para temp Maximas
mod.tx <- filter(d2, serie == 'Tx') %>% mutate(temp = temp-mean(temp, na.rm = T)) %>% pull(temp) %>% 
  ts(start = c(1950, 1), frequency = 365) %>% 
  dlmMLE( parm = 0, build= parMLE)
@
