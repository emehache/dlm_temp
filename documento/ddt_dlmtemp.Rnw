...
load(file = "d1")
load(file = "d2") # datos completos para estanzuela
@
...
La \textbf{distribucion filtrada} (filtering distribution en ingles) consiste en la distribucion del estado no observable $\theta_t$ dada la informacion observada hasta ese mismo momento, es decir $p(\theta_t | y_{1:t}$, se tiene una distribucion para cada momento $t$. La posterior filtrada es de principal interes en aplicaciones secuenciales donde es necesario realizar una nueva estimacion para cada observacion nueva. En el caso de modelos DLM descritos por \eqref{ecu.obs} y \eqref{ecu.evo}, donde operan las restricciones de normalidad y linealidad, es posible tener soluciones cerradas para $p(\theta_t | y_1, \ldots, y_t)$ mediante el Filtro de Kalman (REF??) \textcolor{blue}{es necesario incluir las formulas de recursion para estimar esto??}

El filtro de Kalman puede verse como la resolucion recursiva de un problema de estimacion Bayesiana en el modelo lineal gaussiano, distribucion posterior de interes es 
$p(\theta_t | y_{1:t}$

Suponemos conocida la \textbf{distribucion filtrada} del paso anterior, $p(\theta_{t-1} | y_{1:t-1}) = N(m_{t-1, C_{t-1)$, y la utilizamos para obtener la distribucion previa 
...
dd <- data_frame(t = seq(0,1, length.out = N), w = rnorm(N,0,.1), v = rnorm(N,0,.3) ) %>% 
...
<<graf7b,fig.cap="Ejemplo de un proceso de espacio de estado y su estimación.", fig.height=5>>=
dd %>% 
  select(t, y, theta) %>% 
  mutate(filtro = mod_filter$m[-1], suave=mod_smooth$s[-1], pred1=mod_filter$f )  %>%  
...
<<graf7c,fig.cap="Ejemplo de un proceso de espacio de estado y su estimación.", fig.height=5>>=
dd %>% 
  select(t, y, theta) %>% 
  mutate(filtro = mod_filter$m[-1], suave=mod_smooth$s[-1], pred1=mod_filter$f )  %>%  
  gather(tipo, estimacion, filtro, suave, pred1) %>% 
...
<<graf7,fig.cap="Ejemplo de un proceso de espacio de estado y su estimación.", eval=FALSE>>=
...
<<estima.modelos, echo=T, cache=TRUE>>=
parMLE <- function(par)  dlmModTrig(s = 365,q=2, dV = exp(par), dW = 0)

# Estima el modelo para temp Minimas
mod.tn <- filter(d2, serie == 'Tn') %>% 
  mutate(temp = temp-mean(temp, na.rm = T)) %>% pull(temp) %>% 
  ts(start = c(1950, 1), frequency = 365) %>% 
  dlmMLE( parm = 0, build= parMLE)

# Estima el modelo para temp Maximas
mod.tx <- filter(d2, serie == 'Tx') %>%
...
<<suavizados>>=
# obtener las olas de frio y calor
get_wave <- function(x) {
  # x es un vector de 0 y 1, indicando si un dia cuenta como extremo (frio o calor)
  # hay que identificar rachas de 3 o mas lugares con x=1
  x[is.na(x)] <- 0 # OJO !!!! 
  zz <- rle(x) %>% 
  lapply( as.vector ) %>% bind_cols() %>% 
  mutate( pos2 =  1 + cumsum(lengths), nn = n() ) %>% 
  mutate(pos = c(1, lag(pos2)[-1] ) ) %>% 
  select(-pos2, -nn) %>% 
  filter( values == 1, lengths > 2)
  
  out <- rep(0, length(x) )
  
  for( i in 1:nrow(zz) ) {
    start <- zz$pos[i]; end <- start + zz$lengths[i]-1
    out[start:end] <- i
  }
  out
}

sds_fn <- function(Slist, md) { 
  lapply(Slist, function(x)  md$FF %*% x %*% t(md$FF ) + md$V ) %>% unlist() %>% sqrt()
  }

smooth.tn <- d2 %>% filter(serie == 'Tn') %>% pull(temp) %>% 
  ts(start = c(1950, 1), frequency = 365) %>% 
  dlmSmooth(  parMLE(mod.tn$par) )

smooth.tx <- d2 %>% filter(serie == 'Tx') %>% pull(temp) %>% 
  ts(start = c(1950, 1), frequency = 365) %>% 
  dlmSmooth(  parMLE(mod.tx$par) )

res.wide <- d2 %>% spread(serie, temp) %>% arrange(fecha) %>% 
  mutate( yhat.tn = apply( smooth.tn$s[-1,], 1, function(x)  parMLE(mod.tn$par)$FF %*% x)+mean(Tn, na.rm = T),
          yhat.tx = apply( smooth.tx$s[-1,], 1, function(x)  parMLE(mod.tx$par)$FF %*% x)+mean(Tx, na.rm = T),
          sd.tn = sds_fn(  dlmSvd2var(smooth.tn$U.S[-1], smooth.tn$D.S[-1,]), parMLE(mod.tn$par) ),
          sd.tx = sds_fn(  dlmSvd2var(smooth.tx$U.S[-1], smooth.tx$D.S[-1,]), parMLE(mod.tx$par) )) %>% 
  mutate( dia.frio = as.numeric( (yhat.tx + sd.tx*qnorm(.1) > Tx) & (yhat.tn + sd.tn*qnorm(.1)) > Tn), 
          dia.calor = as.numeric( (yhat.tx + sd.tx*qnorm(.9) < Tx) & (yhat.tn + sd.tn*qnorm(.9)) < Tn) ,
          ola.frio = get_wave(dia.frio), ola.calor = get_wave(dia.calor) )

# zz <- d2 %>% filter(serie == 'Tn') %>% 
#   mutate(mn = mean(temp, na.rm = TRUE), 
#     yhat = apply(smooth.tn$s[-1, c(1,3)], 1, sum) + mn )
# res.long <- d2 %>% filter(serie == 'Tx') %>% 
#   mutate(mn = mean(temp, na.rm = TRUE), 
#          yhat = apply(smooth.tx$s[-1, c(1,3)], 1, sum) + mn ) %>% 
#   bind_rows(zz)
# res.wide <- d2 %>% filter(serie == 'Tx') %>% 
#   mutate(mn = mean(temp, na.rm = TRUE), 
#          yhat.tx = apply(smooth.tx$s[-1, c(1,3)], 1, sum) + mn ) %>% 
#   select( temp, fecha, yhat.tx) %>% 
#   inner_join( select(zz,temp, fecha, yhat),  by='fecha' ) %>% 
#   set_names(nm = c('tx', 'fecha', 'yhat.tx', 'tn', 'yhat.tn') ) %>% 
#   mutate( dia.frio = as.numeric(yhat.tx > tx & yhat.tn > tn), 
#           dia.calor = as.numeric(yhat.tx < tx & yhat.tn < tn ) ) %>% 
#   mutate(ola.frio = get_wave(dia.frio), ola.calor = get_wave(dia.calor) )

@

<<popurri, fig.height=5 >>=
getSeason <- function(input.date){
  ...
  levels(cuts) <- c("Verano","Otoño","Invierno","Primavera","Verano")
  return(cuts)
}
...
xx.ola <- res.wide %>% 
  select(Tn, Tx, fecha, ola.frio, ola.calor) %>% 
  gather(tipo, ola, ola.frio, ola.calor) %>% 
  filter(ola > 0, fecha > make_date(yr, 1, 1), fecha < make_date(yr, 12, 31))

res.wide %>% 
  filter(fecha > make_date(yr, 1, 1), fecha < make_date(yr, 12, 31) ) %>% 
  ggplot() + 
  geom_line(aes(fecha, Tn), color='grey' ) + geom_line(aes(fecha, Tx),color='grey' ) +
  geom_line(aes(fecha, yhat.tn) ) + geom_line(aes(fecha, yhat.tx) ) +
  geom_point(data = xx.ola, aes(fecha, Tn, color= tipo)) + 
  geom_point(data = xx.ola, aes(fecha, Tx, color =tipo)) +
  scale_color_manual(values = c('red', 'blue')) + 
  labs(title = paste('Year', yr, sep =' : '), y = 'temperatura') + 
  theme_bw()
#--  

res.wide %>% 
  mutate(seas = getSeason(fecha), year = year(fecha), month = month(fecha) ) %>% 
  filter(ola.frio > 0) %>% select(Tn, Tx, fecha, seas) %>% 
  gather(serie, temp, Tn, Tx) %>% 
  ggplot() + geom_point(aes(fecha, temp, color = seas) ) + facet_grid(serie~., scale = 'free_y') + 
  labs(y = 'temperatura', color = '', title = 'Estanzuela - Olas de frio') +
  scale_color_brewer(palette = 'Dark2')
  
res.wide %>% 
  mutate(seas = getSeason(fecha), year = year(fecha), month = month(fecha) ) %>% 
  filter(ola.calor > 0) %>% select(Tn, Tx, fecha, seas) %>% 
  gather(serie, temp, Tn, Tx) %>% 
  ggplot() + geom_point(aes(fecha, temp, color = seas) ) + facet_grid(serie~., scale = 'free_y') + 
  labs(y = 'temperatura', color = '', title = 'Estanzuela - Olas de calor') +
  scale_color_brewer(palette = 'Dark2')

describe.olas <- res.wide %>% 
  mutate(seas = getSeason(fecha), year = year(fecha), anoma.tn = Tn - yhat.tn, anoma.tx=Tx-yhat.tx) %>% 
  select(seas, year, anoma.tx, anoma.tn, ola.frio, ola.calor) %>% 
  gather(tipo, ola, ola.frio, ola.calor) %>% 
  group_by(tipo, ola) %>% 
  summarise(seas=first(seas), year=first(year), dura = n(), anoma.tn = mean(anoma.tn), anoma.tx=mean(anoma.tx) ) %>% 
  filter(ola > 0)
  
describe.olas %>% ggplot() + 
  geom_histogram(aes(x=dura, y=..density..) ) + facet_grid(tipo~.) +
  labs(x = 'Duración', y = '')

describe.olas %>% ggplot() + 
  geom_jitter( aes(x=dura, y = anoma.tn, color = seas), height =  0 ) + facet_grid(tipo~., scale = 'free_y') +
  scale_color_brewer(palette = 'Dark2') +
  labs(x = 'Duración', y = 'Anom_TN: Tx-yhat.tx')

describe.olas %>% ggplot() + 
  geom_jitter( aes(x=dura, y = anoma.tx, color = seas), height =  0 ) + facet_grid(tipo~., scale = 'free_y') +
  scale_color_brewer(palette = 'Dark2') + 
  labs(x = 'Duración', y = 'Anom_TX: Tx-yhat.tx')

describe.olas %>% ggplot() + 
  geom_point( aes(x=anoma.tx, y =anoma.tn, color =seas, size=dura) ) + 
  facet_wrap(~tipo  , scale = 'free') +
  scale_color_brewer(palette = 'Dark2') +
  labs(x = 'Anom_TX: Tx-yhat.tx', y = 'Anom_Tn: Tn-yhat.tn')
@
