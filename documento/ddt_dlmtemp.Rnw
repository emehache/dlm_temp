...
res.wide <- d2 %>% spread(serie, temp) %>% arrange(fecha) %>% 
  mutate( yhat.tn = apply( smooth.tn$s[-1,], 1, function(x)  parMLE(mod.tn$par)$FF %*% x)+mean(Tn, na.rm = T),
          yhat.tn2 = apply( smooth.tn$s[-1,], 1, function(x)  parMLE(mod.tn$par)$FF %*% x),
...
# bolazos de resumen ....   
aux <- res.wide %>% 
  filter(fecha > make_date(2010, 1, 1), fecha < make_date(2010, 12, 31))

ggplot() +
  geom_line(data= aux, aes(fecha, Tn) ) + geom_line(data= aux, aes(fecha, Tx) ) + 
  geom_line(data= aux, aes(fecha, yhat.tn),color='blue' ) + geom_line(data= aux, aes(fecha, yhat.tx),color='blue' ) + 
  geom_point(data = filter(aux, ola.frio==1), aes(fecha, Tn), color = 'red' ) +
  geom_point(data = filter(aux, ola.frio==1), aes(fecha, Tx), color = 'red' )

res.wide %>% 
  mutate(year = year(fecha), month = month(fecha) ) %>% 
  filter(ola.frio == 1) %>% 
  group_by(year, month) %>% 
  summarise( ola.calor = sum(ola.calor), ola.frio = sum(ola.frio) ) %>% 
  ggplot() + geom_tile(aes(year,month, fill = ola.frio) ) 

res.wide %>% 
  mutate(year = year(fecha), month = month(fecha) ) %>% 
  filter(ola.frio == 1) %>% 
  group_by(year, month) %>% 
  summarise( tot.ola.frio = sum(ola.frio), 
             temp.frio = mean(Tn, na.rm=T) ) %>% 
  ggplot() + 
  geom_line(aes(year, temp.frio) )  + 
  geom_smooth(aes(year, temp.frio) )  + 
  facet_wrap(~month, scales = 'free_y')

res.wide %>% 
  mutate(year = year(fecha), month = month(fecha) ) %>% 
  group_by(year, month) %>% 
  summarise( tot.ola.calor = sum(ola.calor), tot.ola.frio = sum(ola.frio), 
             temp.frio = mean(Tn * ola.frio, na.rm=T), temp.calor = mean(Tx*ola.calor, na.rm = T) ) %>% 
  ggplot() + geom_line(aes(year, tot.ola.frio) ) + geom_smooth( aes(year, tot.ola.frio) ) + 
  facet_wrap(~month)
@
