...
<<setup,echo=F>>=
opts_chunk$set(echo=FALSE,message=F,warning = F,fig.height=3,fig.pos='p',fig.align='center',highlight = T,cache= T,echo=F)
@
...
\path
(E) edge[right]     node{}         (A)
(A) edge[left]     node[label={[shift={(0.15,-.5)}] \scriptsize{evolución}}] {}         (D)
...
data.frame(t,y,theta,m=mod_filter$m[-1])  %>%  gather(serie,valor,-t) %>% 
  cbind(tipo=factor(c(rep(.5,N),rep(1,N),rep(1,N)))) %>%
  ggplot()+aes(x=t,y=valor,col=serie,alpha=tipo)+geom_line()+
  scale_alpha_discrete(range = c(.5,1),guide=F)
#+scale_color_manual(values=c("theta"='#F8766F',"y"='#7CAE00',"m"="red"),guide=myguide)+theme_bw()
